...
//        }
        else
        {
            ...
            if (_panGest.view.center.x < (_viewCover.frame.size.width + IPAD_CONTENT_WIDTH) / 2)
            {
                [self cover:nil];
            }
            else
...
-(void)reveal:(void(^)(void))completion
{
    ...
    [UIView animateWithDuration:SLIDE_TIMING delay:0 options:UIViewAnimationOptionBeginFromCurrentState
                     animations:^{
                         ...
                         CGRect coverRc = CGRectMake(IPAD_CONTENT_WIDTH, 0, _viewCover.frame.size.width, _viewCover.frame.size.height);
